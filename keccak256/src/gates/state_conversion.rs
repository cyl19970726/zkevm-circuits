...
use crate::gates::tables::{FromBase9TableConfig, FromBinaryTableConfig};
...
impl<F: FieldExt> FromBase9State<F> {
    fn configure(
        meta: &mut ConstraintSystem<F>,
        state: [Column<Advice>; 25],
        table: FromBase9TableConfig<F>,
        ...
        let bi = table.get_base_info(output_b2);
        let bccs: [BaseConversionConfig<F>; 25] = state
            .iter()
            .map(|&lane| {
                BaseConversionConfig::configure(meta, bi.clone(), lane)
